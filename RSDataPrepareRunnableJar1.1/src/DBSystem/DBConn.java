...
public class DBConn {
	...
	public DBConn() {
		System.out.println("DBConn::public DBConn() | ______构造函数");
		this.bIsConnection = false;
	}

	public DBConn(String ip, String port, String mccps_sid, String user,
			String passwd) {
		System.out.println("DBConn::public DBConn( ) | 构造函数_________");
		this.bIsConnection = false;

		// 获取数据库参数选项
		driver = "com.mysql.jdbc.Driver"; // 连接数据库驱动
		this.ip = ip; // 默认数据库服务器地址
		this.port = port; // 默认数据库服务器端口

		this.mccps_sid = mccps_sid; // 默认数据库SID
		this.mccps_user = user; // 默认访问数据库用户
		this.mccps_password = passwd; // 默认访问数据库用户密码
