...
public class WaitOrderExecuted extends Thread {
	...
	public void run() {
		...
		if (this.jobId.contains("L3CP")||this.jobId.contains("L3FP")||this.jobId.contains("L3AP")) {
			// 轮询三级订单库
			if (!doWait(0)) {
				return;
			}
		} else if (this.jobId.contains("L3RN") || this.jobId.contains("L3GN")|| this.jobId.contains("L3DS")) {
			// 轮询四级订单库，直至结束
			if (!doWait(1)) {
				return;
			}
