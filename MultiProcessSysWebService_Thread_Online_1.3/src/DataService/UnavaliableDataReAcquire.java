 ...
 */
public class UnavaliableDataReAcquire {
	// 查询条件
	...
	//无参构造函数
	public UnavaliableDataReAcquire(){
		
	}
	

	// 构造
	public UnavaliableDataReAcquire(String queryCondition, String webserviceUrl,
			String destPath) {
	...
	public long getDataListsValumeSizes(ArrayList<String> dataLists) {
		...
		while (iterator.hasNext()) {
			...
			ArrayList<RSData> rsdataLists=this.rsDataCacheDB.search(" Where dataid='"+dataid+"'");
	...
	public boolean removeHaveCachedDataLists(){
		...
		while (iterator.hasNext()) {
			...
			RSData newDataFromDB=rsdataLists.get(0);
			if (newDataFromDB.dataStatus.equals("Available")) {
				logger.info(dataid+"数据已存在，无需重复请求与下载！");
				//找到feedbackDatalists.datalists对应的数据
				Iterator<RSData> it_feedbackDataList=this.feedbackDatalists.datalists.iterator();
				while (it_feedbackDataList.hasNext()) {
					RSData dataInFeedbackLists = (RSData) it_feedbackDataList.next();
		...
		if (this.feedbackDatalists.datalists.removeAll(needRemovedDatalists)) {
			logger.info(count_Cached+"个数据已经从原来"+sum+"个数据下载列表中移除！剩余"+this.feedbackDatalists.datalists.size()+"个需下载的数据");			
		}
		return true;
	}
	...
	public boolean getFirstAcquireInfos() {
		...
		while (iterator.hasNext()) {
			...
			try {
				...
				// 已经有获取信息的数据
				if (dataStatus.equals("Available")) {
					finishCount++;
					...
					if (dataUrl.startsWith("http")) {
						...
						if (dataUrl.toUpperCase().endsWith("ZIP")&&(!destFile.endsWith(".zip"))) {
							destFile += ".zip";
						}
						// 暂时不用执行下载
...
//					System.out.println(dataFileName+" "+dataUrl);
	...
	public boolean getPollingAcquireInfos() {
		...
		// 下一轮需要继续获取的数据列表
		ArrayList<String> prepareingDatalists = new ArrayList<String>();
		...
		while (iterator.hasNext()) {
			...
			try {
				...
				// 正在获取的数据
					...
					prepareingDatalists.add(curr_dataId);
				}
				// 获取出错的数据
				else {
					errCount++;
		...
		// 更新下一轮需要轮询的数据
		this.prepareingDatalists = prepareingDatalists;
	...
	public static void main(String[] args) throws DocumentException {
		...
		String testDestFilePath = "/dataIO/863_Project/863-Daemon/Project1DataService/DownloadFromRemote/";
		String datas;
		// 查询ID
		String queryId = null;
		// 返回数据条目限制个数
		...
		OutOrderXMLMapSumMain=rSProductKnowledgeDBInstance.InstanceReturn(productName);
...
//			System.out.println("\nkey=" + entry.getKey());
			...
			while (iterator.hasNext()) {
				...
				if (satellite!=null&&sensor!=null) {
					dataPlanTypeLists.add(satellite+"_"+sensor);
				}
			}
...
//		 dataPlanTypeLists.add("HJ1-1A,CCD1");
//		 dataPlanTypeLists.add("HJ1-1A,CCD2");
//		 dataPlanTypeLists.add("HJ1-1A,HSI");
//		 dataPlanTypeLists.add("HJ1-1B,CCD1");
//		 dataPlanTypeLists.add("HJ1-1B,CCD2");
...
////		 dataPlanTypeLists.add("HJ1-1B,IRS");
//		 ////////////////////////////////////////////////////
//		dataPlanTypeLists.add("FY3A,MERSI");
//		dataPlanTypeLists.add("FY3A,VIRR");
//		dataPlanTypeLists.add("FY3B,MERSI");
//		dataPlanTypeLists.add("FY3B,VIRR");
////		/////////////////////////////////////////////////////
//		 dataPlanTypeLists.add("LANDSAT5,TM");
//		 dataPlanTypeLists.add("LANDSAT7,ETM+");
////		 /////////////////////////////////////////////////////
//   	    dataPlanTypeLists.add("AQUA,MODIS");
//		 dataPlanTypeLists.add("TERRA,MODIS");
////		 /////////////////////////////////////////////////////
//		 dataPlanTypeLists.add("MODIS,MODIS_MCD43B1");
//		 dataPlanTypeLists.add("MODIS,MODIS_MCD43C2");
//		 dataPlanTypeLists.add("MODIS,MODIS_MCD43B3");
//		 dataPlanTypeLists.add("MODIS,MODIS_MCD12Q1");
//		 dataPlanTypeLists.add("AQUA,MODIS_MOD03");
//		 dataPlanTypeLists.add("AQUA,MODIS_MOD07");
//		 dataPlanTypeLists.add("TERRA,MODIS_MYD07");
////		 /////////////////////////////////////////////////////////
...
//		 /*
		...
		while (iterator_dataPlan.hasNext()) {
			...
			String satellite = currDataType.split(",")[0];
			String sensor = currDataType.split(",")[1];
			...
			//
			UnavaliableDataReAcquire dataAcquire = new UnavaliableDataReAcquire(testQueryRequest,
					testURL, testDestFilePath);
			// 获取数据列表
...
//			System.out.println(dataAcquire.feedbackDatalists.getCount());			
			...
			if (dataAcquire.prepareingDatalists.size() > 0) {
				...
				do {
					...
					AllDataSize+=dataAcquire.getDataListsValumeSizes(dataAcquire.getDataIdLists());				
